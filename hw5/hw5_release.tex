...
\begin{solution}
	\begin{enumerate}
	    \item 
            \begin{enumerate}
                \item   
                    For a correct class $c$: $L_{cross-entropy}(c) = - \log \frac{\exp(s_c)}{\sum_{k=1}^K\exp(s_k)}$.\\
                    For $2N-1$ classes: $L_{cross-entropy}(c) = - \log \frac{\exp(s_c)}{\sum_{k=1}^{2N-1}\exp(s_k)}$.\\
                    For a pair $p = (i, k)$, Let $s_p = z_i^Tz_k/\tau$ for each $k \neq i$.\\
                    Then, for a positive pair $c = (i, j)$, and negative pairs $(i, k), k\neq i$:\\ $L_{cross-entropy}(i, j) = - \log \frac{\exp(z_i^Tz_j/\tau)}{\sum_{k=1}^{2N-1}\exp(z_i^Tz_k/\tau)} = - \log \frac{\exp(z_i^Tz_j/\tau)}{\sum_{k=1, k\neq i}^{2N}\exp(z_i^Tz_k/\tau)}$,\\
                    since removing $k = i$ from $\{1, ...,2N\}$ leaves exactly $2N-1$ pairs, so the sum ranges are the same.
                \item
                    As $\tau \rightarrow 0^+$, the dot products are massively scaled. So, the "hardest" negative sample will dominate the denominator, leading to $w_k \rightarrow 1$ for that samples, and $w_k \rightarrow 0$ for all other negative samples.\\
                    As $\tau \rightarrow \infty$, the dot products are evened out. So, all samples will get the same weight $w_k = \frac{1}{2N - 1}$.\\
                    Intermediate $\tau$ is preferred such that the weights are proportionally distributed, "harder" samples get more weight, easier ones less, which gives a meaningful signal for training.
            \end{enumerate}
        \item
            \begin{enumerate}
                \item 
                    $$l(i, j) = - \log \frac{\exp(z_i^Tz_j/\tau)}{\sum_{k=1, k\neq i}^{2N}\exp(z_i^Tz_k/\tau)}$$
                    $$l(i, j) = - \log \frac{\exp(1/\tau)}{\sum_{k=1, k\neq i}^{2N}\exp(1/\tau)}$$
                    $$l(i, j) = - \log \frac{\exp(1/\tau)}{(2N-1)\exp(1/\tau)}$$
                    $$l(i, j) = - \log \frac{1}{(2N-1)}$$
                    $$l(i, j) = \log (2N-1)$$
                \item
                    Because the $g$'s goal is to map two augmented views of the same image to the same point in $z$ space, so it throws away information about the features that are augmented (color, rotation, etc.).\\
                    The encoder representation is more useful because it is not directly optimized by the contrastive loss, so it contains richer features that are thrown away by $g$.
                \item
                    Overly weak augmentations will make the task really easy for the model, as it can only use trivial low-level features like pixel similarity, which means that the learned representation won't contain meaningful structure.

                    Overly strong will lead to the positive pairs of images being too distinct or noisy, containing barely any meaningful similarities, while the model has to map them to the same point. This means that the learned representation will be very noisy, again containing little meaningful structure. 
            \end{enumerate}
        \stepcounter{enumi}
        \item \ \\
            \includegraphics[width = 0.5 \linewidth]{hw5/img_output/1.3b.png}
            \includegraphics[width = 0.5 \linewidth]{hw5/img_output/1.3e.png}
	\end{enumerate}
\end{solution}
	...
	\begin{enumerate}
	    \item 
            \begin{enumerate}
                \item   
                    $$\min_G \max_D V(D,G) = \mathbb E_{x\sim p_{\mathrm{data}}}[\log D(x)] + \mathbb E_{z\sim p_z}[\log(1-D(G(z)))]$$
                    $$\max_D V(D,G) = \int_x [p_{\mathrm{data}}(x)\log D(x) + p_g(x)\log(1-D(x) ]dx$$
                    $$\max_D V(D,G) \iff \forall x: \max_{D(x)} [p_{\mathrm{data}}(x)\log D(x) + p_g(x)\log(1-D(x)]$$
                    $$\text{Let }f(D(x)) = p_{\mathrm{data}}(x)\log D(x) + p_g(x)\log(1-D(x)$$
                    $$\frac{d}{dD(x)}f(D(x)) = \frac{p_{data}(x)}{D(x)} - \frac{p_g(x)}{1 - D(x)}$$
                    $$\frac{p_{data}(x)}{D(x)} - \frac{p_g(x)}{1 - D(x)} = 0$$
                    $$p_{data}(x)(1 - D(x)) = p_g(x)D(x)$$
                    $$p_{data}(x) = D(x)(p_g(x) + p_{data}(x))$$
                    $$ D^*(x) = \frac{p_{data}(x)}{p_g(x) + p_{data}(x)}$$
                \item
                    $$\min_G \max_D V(D,G) = \mathbb E_{x\sim p_{\mathrm{data}}}[\log D(x)] + \mathbb E_{z\sim p_z}[\log(1-D(G(z)))]$$
                    $$V(D^*,G) = \int_x [p_{\mathrm{data}}(x)\log (\frac{p_{data}(x)}{p_g(x) + p_{data}(x)})) + p_g(x)\log(1-(\frac{p_{data}(x)}{p_g(x) + p_{data}(x)}) ]dx$$
                    $$\text{Let }m = \frac{1}{2}(p_{data}(x) + p_g(x))$$
                    $$V(D^*,G) = \int_x [p_{\mathrm{data}}(x)[\log (\frac{p_{data}(x)}{m(x)})-\log2] + p_g(x)[\log(\frac{p_{g}(x)}{m(x)}) - \log2]]dx$$
                    $$V(D^*,G) = \int_x [p_{\mathrm{data}}(x)\log (\frac{p_{data}(x)}{m(x)}) + p_g(x)\log(\frac{p_{g}(x)}{m(x)}) - 2\log 2]dx$$
                    $$V(D^*,G) = KL(p_{data}||m)+KL(p_g||m)- \log 4$$
                    $$V(D^*,G) = 2(\frac{1}{2}KL(p_{data}||m)+\frac{1}{2}KL(p_g||m))- \log 4$$
                    $$V(D^*,G) = 2 JSD(p_{data}||p_g)- \log 4$$
                \item
                    JSD measures the distance between distributions. This means that with an optimal discriminator, the GAN objective is equivalent to minimizing the distance between the $p_{data}$ and $p_g$ distributions. This means that the generator is trying to generate samples such that they mirror the true distribution of the data.
            \end{enumerate}
        \item
            \begin{enumerate}
                \item 
                    $$\max_D V(D,G) = \mathbb E_{x\sim p_{\mathrm{data}}}[\log D(x)] + \mathbb E_{z\sim p_z}[\log(1-D(G(z)))]$$
                \item
                    $$\min_G V(D,G) = \mathbb E_{z\sim p_z}[\log(1-D(G(z)))]$$
                \item
                    The training loop alternates between 1) training the discriminator as a binary classifier to accurately predict whether a sample is positive or negative (real or generated), and 2) training the generator to generate "hard" negative samples that "fool" the discriminator (it predicts they're real).
            \end{enumerate}
        \item \ \\
            \includegraphics[width = 0.5 \linewidth]{hw5/img_output/2.3a.png}
            \includegraphics[width = 0.5 \linewidth]{hw5/img_output/2.3b.png}
	\end{enumerate}
\end{solution}
...
\begin{solution}
	\begin{enumerate}
	    \stepcounter{enumi}
        \item 
            $$\includegraphics[width = 0.5 \linewidth]{hw5/img_output/3.2.png}$$
        \item
            $$\includegraphics[width = 0.5 \linewidth]{hw5/img_output/3.3.png}$$
        \item
            $$\includegraphics[width = 0.5 \linewidth]{hw5/img_output/3.4.png}$$
            Both the standardized and unstandardized cases produce recognizable digits as cluster means, and across the 3 iterations, find all 10 digit classes. The standardized centroids are blurrier, which makes sense since the standardization process scales up low-variance pixels, like edges and corners that are always 0, essentially adding noise to the images.
        \item
            $$\includegraphics[width = 0.5 \linewidth]{hw5/img_output/3.5.png}$$
            Max linkage produces the blurriest clusters. This is because it doesn't chain as explained in 6), and so each cluster will have more representative sample size, and so the mean image won't be as crisp.
            Min linkage produces crisp clusters and so does centroid. This is because almost all samples are under 1 cluster and the rest of the clusters have only a couple points, so the mean will be very crisp\\
            HAC only needs 1 run because it's deterministic - it doesn't depend on initialization like k-means.
        \item
            $$\includegraphics[width = 0.5 \linewidth]{hw5/img_output/3.6a.png}\includegraphics[width = 0.5 \linewidth]{hw5/img_output/3.6b.png}$$
            The plots show the problem with min-linkage, which is that it prefers elongated clusters. It is not the right fit for this data set, as it is better suited for non-convex class structure. Here, all the images are very similar since they are just black and white pixels of digits so it chains all of them together.\\
            Centroid faces the same problem since the data is so similar, that the mean of the biggest cluster will be close to the next point and it will pull everything into one.\\
            Max-linkage doesn't face this problem since it prefers compact clusters, and that's why each cluster has a more representative sample size.\\
        \item
            $$\includegraphics[width = \linewidth]{hw5/img_output/3.7.png}$$
            The closest HAC variant to K-Means is max-linkage. This exactly agrees with the explanation from the previous part - min-linkage and centroid put all samples into 1 cluster, while max-linkage spreads them out more evenly, just like k-means.
        \item
            Clustering is not ideal for digit identification. Even though K-means recovers clusters that look like digits, it has no guarantee of finding the 10 true classes, which is what we saw in the graphs.\\
            Since clustering is unsupervised, they are not expected to match the true labels. It just groups data by similarity and there is no loss on the cluster identity matching the true label.\\
            Outliers are problematic especially for max-linkage. Data with class imbalance would be problematic, because one class can dominate others and get assigned multiple clusters and rare classes might be grouped together. Ambiguos digits (i.e. 4 - 9, 1 - 7) will sit between clusters and get assigned arbitrarily since there is no punishment on them not matching the true class.
	\end{enumerate}
\end{solution}
