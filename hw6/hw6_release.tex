...
\begin{solution}
	\begin{enumerate}
	    \item
            \begin{enumerate}
                \item 
                    $$\alpha_t(z_t) = p(x_{0:t}, z_t)$$
                    $$p(z_t|x_{0:t}) = p(x_{0:t}, z_t) / p(x_{0:t})$$
                    $$p(z_t|x_{0:t}) = \alpha_t / p(x_{0:t}) \propto \alpha_t$$
                    $$\text{We are conditioning on observations only up to time $t$, so the quantity does not relate to $\beta_t(z_t)$,}$$
                    $$\text{as that relates to future observations. The operation is called filtering.}$$
                \item
                    $$x_t = z_t + \gamma_t$$
                    $$\gamma_t \sim N(0, \sigma_{\gamma}^2)$$
                    $$p(x_t|z_t) = N(z_t, \sigma_{\gamma}^2)$$
                \item
                    $$p(z_t|x_{0:t-1}) = \int p(z_t, z_{t-1}|x_{0:t-1})\ dz_{t-1}$$
                    $$p(z_t|x_{0:t-1}) = \int p(z_t| z_{t-1},x_{0:t-1})p(z_{t-1}|x_{0:t-1})\ dz_{t-1}$$
                    $$z_t = z_{t-1} + \epsilon_{t-1}, \epsilon_{t-1} \sim N(0, \sigma_\epsilon^2),$$
                    $$p(z_t|x_{0:t-1}) = \int N(z_t; z_{t-1}, \sigma_\epsilon^2)N(z_{t-1}; \mu_{t-1}, \sigma_{t-1}^2)\ dz_{t-1}$$
                    $$p(z_t|x_{0:t-1}) = \int N(z_t -z_{t-1}; 0, \sigma_\epsilon^2)N(z_{t-1}; \mu_{t-1}, \sigma_{t-1}^2)\ dz_{t-1}$$
                    $$p(z_t|x_{0:t-1}) = N(z_t; \mu_{t-1}, (\sigma_\epsilon^2 + \sigma_{t-1}^2))$$
                \item
                    $$p(z_t|x_{0:t})\propto p(x_t|z_t)p(z_t|x_{0:t-1})$$
                    $$p(z_t|x_{0:t})\propto N(x_t;z_t, \sigma_{\gamma}^2)N(z_t; \mu_{t-1}, (\sigma_\epsilon^2 + \sigma_{t-1}^2))$$
                    $$p(z_t|x_{0:t})\propto N(z_t; x_t, \sigma_{\gamma}^2)N(z_t; \mu_{t-1}, (\sigma_\epsilon^2 + \sigma_{t-1}^2))$$
                    $$p(z_t|x_{0:t}) \propto N\left(z_t; \frac{\sigma_\epsilon^2 + \sigma_{t-1}^2}{\sigma_{\gamma}^2+\sigma_\epsilon^2 + \sigma_{t-1}^2}x_t + \frac{\sigma_{\gamma}^2}{\sigma_{\gamma}^2+\sigma_\epsilon^2 + \sigma_{t-1}^2}\mu_{t-1}, \ \left(\frac{1}{\sigma_{\gamma}^2} + \frac{1}{\sigma_\epsilon^2 + \sigma_{t-1}^2}\right)^{-1}\right)$$\\
                    $$\mu_t = \frac{\sigma_\epsilon^2 + \sigma_{t-1}^2}{\sigma_{\gamma}^2+\sigma_\epsilon^2 + \sigma_{t-1}^2}x_t + \frac{\sigma_{\gamma}^2}{\sigma_{\gamma}^2+\sigma_\epsilon^2 + \sigma_{t-1}^2}\mu_{t-1}$$
                    $$\sigma_t^2 = \left(\frac{1}{\sigma_{\gamma}^2} + \frac{1}{\sigma_\epsilon^2 + \sigma_{t-1}^2}\right)^{-1}$$   
            \end{enumerate}
        \item
            Since $\frac{\sigma_\epsilon^2 + \sigma_{t-1}^2}{\sigma_{\gamma}^2+\sigma_\epsilon^2 + \sigma_{t-1}^2} + \frac{\sigma_{\gamma}^2}{\sigma_{\gamma}^2+\sigma_\epsilon^2 + \sigma_{t-1}^2} = 1$, $\mu_t$ is a weighted average of the current observation $x_t$ and the previous hidden state's mean $\mu_{t-1}$, which we interpret as a summary of all past observations. They are weighted inversely by their own variance - noisier observation leads to  more weight given to the summary and less to the observation itself, and vice versa. 
	\end{enumerate}
\end{solution}
	...
	\begin{enumerate}
	    \item 
            \begin{enumerate}[start = 3]
                \item
                    $$\includegraphics[width = 0.5 \linewidth]{hw6/img_output/2.1c-i.png} \includegraphics[width = 0.5 \linewidth]{hw6/img_output/2.1c-ii.png}$$
                    $$\includegraphics[width = 0.5 \linewidth]{hw6/img_output/2.1c-iii.png} \includegraphics[width = 0.5 \linewidth]{hw6/img_output/2.1c-iv.png}$$
                \item
                    $$\includegraphics[width = 0.5 \linewidth]{hw6/img_output/2.1d.png}$$
                    It takes 5 iterations to converge with $ct = 0.01$. It also takes 5 iterations with $ct = 0.001$ and $ct = 0.0001$. This is because we find the optimal policy at iteration 4, and so iteration 5 evaluates the same policy. Our policy evaluation algorithm always starts at the same initialization, and so will always converge to the same value function. The difference in the value function at iterations 4 and 5 will thus be always 0, which will satisfy any positive $ct$. 
            \end{enumerate}
        \item
            \begin{enumerate}[start = 2]
                \item 
                    $$\includegraphics[width = 0.5 \linewidth]{hw6/img_output/2.2b-i.png} \includegraphics[width = 0.5 \linewidth]{hw6/img_output/2.2b-ii.png}$$
                    $$\includegraphics[width = 0.5 \linewidth]{hw6/img_output/2.2b-iii.png} \includegraphics[width = 0.5 \linewidth]{hw6/img_output/2.2b-iv.png}$$
                \item
                    $$\includegraphics[width = 0.5 \linewidth]{hw6/img_output/2.2c.png}$$
                    With $ct = 0.01$, it takes 25 iterations to converge. It takes 31 iterations for $ct = 0.001$, and 38 for $ct = 0.0001$. In value iteration, we directly update the value function and use it for the convergence check at each step. Every iteration is an extra step of exploration, which brings us closer to the optimal value function. But this information is discounted by gamma, and so every iteration will change V by 70\% as much as the previous did, and so roughly 6.5 iterations are needed to get a factor of 10 tighter accuracy, which is the pattern we see. 
            \end{enumerate}
        \item
            $$\includegraphics[width = 0.5 \linewidth]{hw6/img_output/2.3-i.png} \includegraphics[width = 0.5 \linewidth]{hw6/img_output/2.3-ii.png}$$
            $$\includegraphics[width = 0.5 \linewidth]{hw6/img_output/2.3-iii.png} \includegraphics[width = 0.5 \linewidth]{hw6/img_output/2.3-iv.png}$$
            The graphs are from left to right according to $\gamma$ from $0.6$ to $0.9$. When the future rewards are less discounted, the policy prioritizes getting to the high reward spot ($+50$) as soon as possible to maximize long-term gain, even if its through more negative reward spots $(-20, -50)$, taking a short-term loss. The higher the gamma, the less risk-averse the policy is thanks to the long-term gain possibility. The lower the gamma, the more short-term oriented the model is, even choosing to get to the the $(+4)$ square when close to it, since the future is so discounted.
        \item
            Overall, the model will become much more risk averse, avoiding short-term losses more, since the repeated long-term gain is taken away.\\
            With very high gamma, I would expect the policy to take the least negative route to the highest yield square, no matter the distance. It would try to avoid landing in other positive squares that earn less, since it can get a higher reward elsewhere.\\
            With low gamma, the model will become more short-term oriented, and will simply seek the nearest positive square, not willing to take any negative routes.
        \item
            Our approach is very efficient, since we don't need to sample the environment like in RL to figure out its dynamics. We can also get extremely accurate solutions by having the exact model of the environment.\\
            We have to consider how we came up with our model; if our model is wrong, then so will our solutions. RL is able to learn the environment from real experience. Reward design, which also applies to RL, is crucial - we need to design a reward signal that actually drives the objective. If we didn't accurately do that, then our solution will be wrong for the goal no matter the approach.
	\end{enumerate}
\end{solution}
...
\begin{solution}
	First, I implemented the vanilla version and played around with the parameters and wasn't getting very good results. So I followed the advice and added epsilon decay. I assumed I will start from pure random exploration, and tried to find a decay that will make my agent perform well:
    $$\includegraphics[width = 0.5 \linewidth]{hw6/img_output/3-i.png}$$
    It's clear that the faster decay, the earlier the agent starts getting good results but has very high variance, and if the decay is too slow, then we still have too much randomness by the end. $decay = 0.9$ seemed like a great middle ground.\\
    $$\includegraphics[width = 0.5 \linewidth]{hw6/img_output/3-ii.png}\includegraphics[width = 0.5 \linewidth]{hw6/img_output/3-iii.png}$$
    I also tried alphas and gammas in the same way. With alpha too small, the learning rate is too slow for the model to produce results, and with high alpha, the variance is too high, as one bad outcome has too much weight. $alpha = 0.1$ seemed like the best balance. With small gamma, the agent doesn't optimize for long streaks but rather for immediate results, so it never reaches a very high score. With gamma too high, results are too noisy, as the agent can rely too much on random future outcomes. $gamma = 0.9$ seemed like a good middle ground.
\end{solution}
