...
\begin{solution}
	\begin{enumerate}
        \item
            \begin{enumerate}
                \item 
                    $$Q = XW_Q = \begin{pmatrix} 1 & 0 \\ 0 & 1 \end{pmatrix}\begin{pmatrix} 1 & 1 \\ 0 & 1 \end{pmatrix} = \begin{pmatrix} 1 & 1 \\ 0 & 1 \end{pmatrix}$$
                    $$K = XW_K = \begin{pmatrix} 1 & 0 \\ 0 & 1 \end{pmatrix}\begin{pmatrix} 1 & 0 \\1 & 1 \end{pmatrix} = \begin{pmatrix} 1 & 0 \\ 1 & 1 \end{pmatrix}$$
                    $$V = XW_V = \begin{pmatrix} 1 & 0 \\ 0 & 1 \end{pmatrix}\begin{pmatrix} 2 & 0 \\0 & 1 \end{pmatrix} = \begin{pmatrix} 2 & 0 \\ 0 & 1 \end{pmatrix}$$
                    $$S = QK^T = \begin{pmatrix} 1 & 1 \\ 0 & 1 \end{pmatrix}\begin{pmatrix} 1 & 0 \\ 1 & 1 \end{pmatrix}^T = \begin{pmatrix} 1 & 1 \\ 0 & 1 \end{pmatrix}\begin{pmatrix} 1 & 1 \\ 0 & 1 \end{pmatrix} = \begin{pmatrix} 1 & 2 \\ 0 & 1 \end{pmatrix}$$
                \item 
                    $$S/ \sqrt{d_k} = S/ \sqrt{2} = \begin{pmatrix} 1 & 2 \\ 0 & 1 \end{pmatrix} / \sqrt{2} = \begin{pmatrix} \sqrt{2} /2 & \sqrt{2} \\ 0 & \sqrt{2}/2 \end{pmatrix}$$
                    $$A = \begin{pmatrix} \frac{e^{\sqrt{2}/2}}{e^{\sqrt{2}/2} + e^{\sqrt{2}}} & \frac{e^{\sqrt{2}}}{e^{\sqrt{2}/2} + e^{\sqrt{2}}} \\ \frac{1}{1 + e^{\sqrt{2}/2}} & \frac{e^{\sqrt{2}/2}}{1 + e^{\sqrt{2}/2}} \end{pmatrix}$$
            \end{enumerate}
        \item
            \begin{enumerate}
                \item 
                    $$E[q^Tk] = \sum_{i=1}^{d_k}E[q_i]E[k_i] = \sum_{i=1}^{d_k}0 = 0$$
                    $Var[q^Tk] = \sum_{i=1}^{d_k}Var[q_ik_i] = \sum_{i=1}^{d_k}E[q_i^2k_i^2] - (E[q_ik_i])^2 = \sum_{i=1}^{d_k}E[q_i^2]E[k_i^2] = \sum_{i=1}^{d_k}(Var[q_i] + (E[q_i])^2)(Var[k_i] + (E[k_i])^2) = \sum_{i=1}^{d_k} 1 = d_k$
                \item
                    The magnitude of $q^Tk$ increases which is illustrated by the sum $\sum_{i=1}^{d_k}q_ik_i$; more terms in the sum leads to higher magnitude.\\
                    This means that each term in $S$ will be of larger magnitude. Since softmax is exponential, one token will get almost all the weight and others almost none, becoming more peaked, which will lead to vanishing gradients $a_i(1 - a_i)$ during training.
                \item
                    $$Var[q^Tk] = d_k$$
                    $$Var[1/ \sqrt{d_k} * q^Tk] = 1/d_k * Var[q^Tk] = 1$$
            \end{enumerate}
        \item
            \begin{enumerate}
                \item
                    $$Q' = P_{\sigma}XW_Q = P_{\sigma}(XW_Q) = P_{\sigma}Q, K' = P_{\sigma}K, V' = P_{\sigma}V$$
                    $$S' = Q'(K')^T = P_{\sigma}Q(P_{\sigma}K)^T = P_{\sigma}QK^TP_{\sigma}^T = P_{\sigma}SP_{\sigma}^T$$
                    $softmax(S'/\sqrt{d_k})  = softmax(P_{\sigma}SP_{\sigma}^T / \sqrt{d_k}) = P_{\sigma}softmax(S\sqrt{d_k})P_{\sigma}^T$, since softmax is applied row-wise
                    $$Attention(Q', K', V') = P_{\sigma}softmax(S\sqrt{d_k})P_{\sigma}^T * P_{\sigma}V = P_{\sigma}softmax(S\sqrt{d_k})V = P_{\sigma} Attention(Q, K, V)$$
                \item
                    The meaning of language depends on the order in which words appear. If self-attention is permutation equivariant, this means that we would get the same self-attention (except shuffled) even if we shuffled the order of words. The model would thus pay no attention to word ordering during training, which is wrong for understanding language.
                \item
                    This is because $PE$ is will be unchanged upon permuting $X$: $X' = P_{\sigma}X + PE \neq P_{\sigma} (X + PE)$, and therefore self-attention is no longer permutation equivariant, and so it captures information about position of tokens.
            \end{enumerate}
        \stepcounter{enumi}
        \item
            $$\includegraphics[width = 0.5 \linewidth]{hw4/img_output/1.5e.png}$$
            The heatmap confirms that the [CLS] token attends to VAL0, which is inherently the behavior we want since the classification depends on that.
...
\begin{solution}
	\begin{enumerate}
	    \item 
            \begin{enumerate}
                \item 
                    $$\includegraphics[width = 0.5 \linewidth]{hw4/img_output/2.1a1.png}$$
                    $$\includegraphics[width = \linewidth]{hw4/img_output/2.1a2.png}$$
                \item
                    $$\includegraphics[width =\linewidth]{hw4/img_output/2.1b.png}$$
                    The autoencoder is trained to encode useful features into the latent space and then reconstruct the image from them. If we sample random vectors in the latent space following a normal, they never went through the encoder, so the decoder has no reason to produce coherent outputs from them, as it was only trained on vectors the encoder produced.
            \end{enumerate}
        \item
            \begin{enumerate}
                \item 
                    Since $z$ is high dimensional, it is almost impossible to randomly sample $z$ following a normal that actually explain $x$. Almost all samples will just be random noise which will contribute nothing to the integral, and so we would need an unfeasible amount of samples.
                \item
                    \begin{enumerate}
                        \item 
                            $$\KL\!\big(q_\phi(\bz \mid \bx) \,\|\, p(\bz \mid \bx)\big) = \E_{q_\phi(\bz \mid \bx)}\!\Big[\log q_\phi(\bz \mid \bx) - \log p(\bz \mid \bx)\Big]=$$
                            $$= \E_{q_\phi(\bz \mid \bx)}\!\Big[\log q_\phi(\bz \mid \bx) - \log\frac{ p(x,z)}{p(x)}] = $$
                            $$= \E_{q_\phi(\bz \mid \bx)}\!\Big[\log q_\phi(\bz \mid \bx) - \log p(x,z) + \log p(x)]$$
                            $$ = \log p(x) + \E_{q_\phi(\bz \mid \bx)}\!\Big[\log q_\phi(\bz \mid \bx) - \log p(x,z)\Big]$$
                            $$\log p(\bx) = \KL\!\big(q_\phi(\bz \mid \bx) \,\|\, p(\bz \mid \bx)\big) - \E_{q_\phi(\bz \mid \bx)}\!\Big[\log q_\phi(\bz \mid \bx) - \log p(x,z)\Big] = $$
                            $$= \KL\!\big(q_\phi(\bz \mid \bx) \,\|\, p(\bz \mid \bx)\big) + \E_{q_\phi(\bz \mid \bx)}\!\Big[\log p(\bx, \bz) - \log q_\phi(\bz \mid \bx)\Big].$$
                        \item
                            Since $ELBO \leq \log p(x)$, it is a lower bound for the log-likelihood, and thus maximizing it is a good substitute for maximizing the true value.
                        \item
                            $$\mathrm{ELBO} = \E_{q_\phi(\bz \mid \bx)}\!\Big[\log p(\bx, \bz) - \log q_\phi(\bz \mid \bx)\Big] = $$
                            $$ = \E_{q_\phi(\bz \mid \bx)}\!\Big[\log p(\bx | \bz) + \log p(z) - \log q_\phi(\bz \mid \bx)\Big] = $$
                            $$\E_{q_\phi(\bz \mid \bx)}\!\Big[\log p(\bx | \bz)\Big] - \E_{q_\phi(\bz \mid \bx)} \Big[\log q_\phi(\bz \mid \bx) - \log p(z)\Big]=$$
                            $$= \E_{q_\phi(\bz \mid \bx)}\!\big[\log p(\bx \mid \bz)\big] - \KL\!\big(q_\phi(\bz \mid \bx) \,\|\, p(\bz)\big)$$
                    \end{enumerate}
                \item
                    \begin{enumerate}
                        \item
                            Minimizing the negative expected log-likelihood is the same as maximizing the expected log-likelihood, which increases the accuracy of the decoder reconstructions of $x$ from $z$. This is the same goal as minimizing the reconstruction loss from section 1.
                        \item
                            The original autoencoder only had a goal of encoding useful information into z and accurately reconstructing from it, and so random samples from z following a normal distribution were unlikely to contain useful information for coherent decodings. Now with the penalization, the autoencoder is incentivized to encode useful information into z whilst following a normal distribution, so if we randomly sample from it, the decodings are likely to lead to coherent outputs.
                        \item
                            The first term wants the autoencoder to reconstruct as well as possible, and so it pushes the encoder to encode useful information into $z$ so that the decoder can accurately reconstruct, while the second term pushes $z$ to follow random noise. This will likely lead to not as accurate reconstructions (blurry, some features slightly off), but also the decoder will be able to produce coherent outputs if we sample from $z$ following a normal distribution.
                    \end{enumerate}
            \end{enumerate}
                \item
                    \begin{enumerate}
                        \item 
                            Sampling is a stochastic action, and we can't find a gradient in randomness, so we can't backpropagate through it.\\
                            If $\epsilon \sim N(0, I)$, then $z = \mu_\phi(x) + \sigma_\phi(x) \odot \epsilon \sim N(\mu_\phi(x), diag(\sigma_\phi^2(x))$ since we are just scaling a gaussian distribution. We can now backpropagate through $z$ since everything that is parametrized is deterministic and the random noise has no parameters.
                        \item
                            $$\KL\!\big(q\,\|\, p\big) = \E_q\!\Big[\log q - \log p\Big]=$$
                            $$= \E_{q}\!\Big[\log [\prod_{j=1}^d q(z_j)] - \log [\prod_{j=1}^d p(z_j)] = \Big]$$
                            $$=\sum_{j=1}^d \E_{q}\!\Big[ \log q(z_j) - \log p(z_j) \Big] =$$
                            $$= \sum_{j=1}^d \E_{q}[-\log(\sigma_j) - \frac{1}{2}\log(2\pi) - \frac{(z_j - \mu_j)^2}{2\sigma_j^2} + \frac{1}{2}\log(2\pi) + \frac{z_j^2}{2}]$$
                            $$= \sum_{j=1}^d[-\log(\sigma_j) - \frac{\sigma_j^2}{2\sigma_j^2} + \frac{\mu_j^2 + \sigma_j^2}{2}]$$
                            $$= \sum_{j=1}^d[-\frac{1}{2}\log(\sigma_j^2) - \frac{1}{2} +  \frac{\mu_j^2 + \sigma_j^2}{2}]$$
                            $$= \frac{1}{2}\sum_{j=1}^d(\mu_j^2 + \sigma_j^2-\ln(\sigma_j^2) - 1)$$
                        \item $$\includegraphics[width = 0.5 \linewidth]{hw4/img_output/2.3c1.png} \includegraphics[width = 0.5 \linewidth]{hw4/img_output/2.3c2.png}$$
                        $$\includegraphics[width = 0.5 \linewidth]{hw4/img_output/2.3c3.png}$$
                        $$\includegraphics[width = \linewidth]{hw4/img_output/2.3c4.png}$$
                        $$\includegraphics[width =  \linewidth]{hw4/img_output/2.3c5.png}$$
                        The decoded samples look like faces now. In 1(b), they looked like just random green and brown pixels, so this is a big change.
                    \end{enumerate}
	\end{enumerate}
\end{solution}
\newpage
...
\begin{solution}
	\begin{enumerate}
	    \item 
            \begin{enumerate}
                \item 
                    $$P(X \leq \frac{B}{2}) = P(\bar Z \leq \frac{1}{2}) = $$
                    $$= P(\bar Z -p\leq \frac{1}{2} - p) =$$
                    $$= P(\bar Z -E[\bar Z]\leq - ( p - \frac{1}{2}) =$$
                    $$= P(\bar Z -E[\bar Z]\leq - t ) \leq$$
                    $$\leq \exp\!\!\left(\frac{-2B^2 t^2}{\sum_{i=1}^{B}(b_i - a_i)^2}\right) =$$
                    $$= \exp\!\!\left(\frac{-2B^2 (p - \frac{1}{2})^2}{B}\right), \text{since $b_i = 1, a_i = 0$}$$
                    $$= \exp\!\!\left(-2B (p - \frac{1}{2})^2\right)$$
                \item
                    $p = 0.6$\\
                    $B = 10: 0.819$\\
                    $B = 100: 0.135$\\
                    $B = 1000: 2.06 \times 10^{-9}$\\
                    $10^{-6} = e^{(-2B(0.1)^2)} \rightarrow B \approx 690.78$, so it drops below $10^{-6}$ at $B = 691$.\\
                    It looks like it's pretty good if the trees are not incredibly deep; I imagine this results in fewer parameters than a deep NN, so very achievable.\\
                    It's probably not necessary since we are computing an upper bound, and the error in reality is likely much smaller.
                \item
                    It weakens it because correlated trees will make the same errors at the same time, and so they are likely to fail the majority vote. Independent trees make different errors, and thus the majority vote is more likely to pass.\\
                    This reduces correlation because trees only see a random subset of the features, and so they will build different decision boundaries and make different errors. This improves convergence in turn as explained above.
            \end{enumerate}
        \item
            \begin{enumerate}
                \item 
                    $$\text{Var}\!\left(\frac{1}{B}\sum_{b=1}^{B} T_b(\mathbf{x})\right) = \frac{1}{B^2}\text{Var}\!\left(\sum_{b=1}^{B} T_b(\mathbf{x})\right) = $$
                    $$= \frac{1}{B^2}\!\left(\sum_{i=1}^{B}\sum_{j=1}^{B} Cov(T_i, T_j)\right) =$$
                    $$= \begin{cases} 
                        Var(T_i) = \sigma^2 & \text{if } i=j \\
                    ...
                    $$=\frac{(1 - \rho)\sigma^2}{B} + \rho\sigma^2$$
                    \item
                        \begin{enumerate}
                            \item 
                                It converges to $\rho\sigma^2$, and so we can't make the variance arbitrarily small by adding more trees, only by reducing correlation between trees.
                            \item
                                When $\rho = 0$, the trees are independent, and so the variance of the ensemble becomes $\frac{\sigma^2}{B}$, which approaches 0 as $B \rightarrow\infty$, which is the ideal scenario.
                                When $\rho = 1$, the trees are perfectly correlated, and so the variance of the ensemble becomes ${\sigma^2}$, which is independent of $B$ since all the trees will predict the same outcome, and adding more won't improve variance.
                        \end{enumerate}
                    \item
                        If $m = 1$, then the variance $\sigma^2$ will be high and so will be the total variance even tho $\rho$ is minimized. This is because each tree only sees 1 random feature and so the predictions will be quite useless and all over the place. $m$ thus controls the tradeoff between $\rho$ and $\sigma^2$.
            \end{enumerate}
        \item
            Increasing model capacity for random forests will linearly increase computational cost, since all trees always run. Increasing model capacity for MoE increases computational cost linearly in terms of how many experts are activated for an input on average. This is exactly why Mixtral has ~47B parameters but only activates ~13B per token, since only a few experts are activated per input. This is impossible for random forests, since all ~47B parameters would be activated for any input, and that's why they can't scale up the way MoE can.
	\end{enumerate}
\end{solution}
