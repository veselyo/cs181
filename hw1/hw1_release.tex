...
\begin{solution}
	\begin{enumerate}
        ...
        \item For $L_{lasso}$, the gradient \[\frac{d}{dw}||w||_1 = \begin{cases} +1 & \text{if } w_d > 0 \\ -1 & \text{if } w_d < 0 \\ \text{undefined} & \text{if } w_d = 0 \end{cases},\] and so it's no longer to find a closed form solution since the gradient depends on the values $w_d$ themseleves, unlike in $L_{ridge}$ where we had a linear function in terms of $w$ for the gradient and so we could set it to 0 and solve for any $w$.
	\end{enumerate}
\end{solution}
