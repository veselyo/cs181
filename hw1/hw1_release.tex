...
\begin{solution}
	\begin{enumerate}
        ...
        \item \begin{enumerate}
            ...
            \item Both methods require storing the entire training set $N$ to be able to either determine which are the nearest $k$ neighbors, or to assign kernel weights for each training point, so space complexity is $O(N)$.\\
            Time complexity is also $O(N)$ for both methods, since kNN has to compute all distances from $x^*$ to all training points to get nearest $k$ neighbors, and kernelized regression needs kernel weights between $x^*$ and each training point.
