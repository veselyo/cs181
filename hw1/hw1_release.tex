...
\begin{solution}
	\begin{enumerate}
        ...
        \item \begin{enumerate}
            ...
            \item Test MSE for $\tau = 1: 1.95, \tau = 50: 1.86, \tau = 2500: 8.33$, so $\tau = 50$ gives us lowest MSE. Conceptually, $\tau = 1$ gives us a very similar curve to that of $k = 1$ with kNN, and so the predictions are just nearest neighbors. With $\tau = 50$, it's still small enough that the curve is very reliant on the nearest points, but also has higher bandwith to capture the genral trend of the curve rather than relying on a single point, which is why it has a lower MSE than. 
        \end{enumerate} 
